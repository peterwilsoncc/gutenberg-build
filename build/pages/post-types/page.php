...
global $gutenberg_post_types_routes, $gutenberg_post_types_menu_items;
$gutenberg_post_types_routes     = array();
$gutenberg_post_types_menu_items = array();

/**
 * Register a route for the post-types page.
 *
 ...
 */
function gutenberg_register_post_types_route( $path, $content_module = null, $route_module = null ) {
	global $gutenberg_post_types_routes;
	...
	$gutenberg_post_types_routes[] = $route;
}

/**
 * Register a menu item for the post-types page.
 *
 ...
 */
function gutenberg_register_post_types_menu_item( $id, $label, $to, $parent_id = '', $parent_type = '' ) {
	global $gutenberg_post_types_menu_items;
	...
	$gutenberg_post_types_menu_items[] = $menu_item;
}

/**
 * Get all registered routes for the post-types page.
 *
 * @return array Array of route objects.
 */
function gutenberg_get_post_types_routes() {
	global $gutenberg_post_types_routes;
	return $gutenberg_post_types_routes ?? array();
}

/**
 * Get all registered menu items for the post-types page.
 *
 * @return array Array of menu item objects.
 */
function gutenberg_get_post_types_menu_items() {
	global $gutenberg_post_types_menu_items;
	return $gutenberg_post_types_menu_items ?? array();
}

/**
 * Preload REST API data for the post-types page.
 * Automatically called during page rendering.
 */
function gutenberg_post_types_preload_data() {
 ...
 */
function gutenberg_post_types_render_page() {
	...
	gutenberg_post_types_preload_data();

	// Get all registered routes and menu items
	$menu_items = gutenberg_get_post_types_menu_items();
	$routes = gutenberg_get_post_types_routes();
	...
	if ( file_exists( $asset_file ) ) {
		...
		wp_register_script( 'post-types-prerequisites', '', $asset['dependencies'], $asset['version'], true );

		// Add inline script to initialize the app
		$init_modules = [];
		wp_add_inline_script(
			'post-types-prerequisites',
			sprintf(
		...
		wp_register_style( 'post-types-prerequisites', false, $style_dependencies, $asset['version'] );

		// Build dependencies for post-types module
		...
		wp_enqueue_script( 'post-types-prerequisites' );
		wp_enqueue_script_module( 'post-types' );
		wp_enqueue_style( 'post-types-prerequisites' );
	}
	...
	<body class="post-types">
		<div id="post-types-app" style="height: 100vh; box-sizing: border-box;"></div>
	<?php
 ...
 */
function gutenberg_post_types_intercept_render() {
	// phpcs:ignore WordPress.Security.NonceVerification.Recommended
	if ( isset( $_GET['page'] ) && 'post-types' === $_GET['page'] ) {
		gutenberg_post_types_render_page();
		exit;
	}
}

// Hook the interceptor to admin_init
add_action( 'admin_init', 'gutenberg_post_types_intercept_render' );
