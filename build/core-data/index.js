...
(wp ||= {}).coreData = (() => {
  ...
  function defaultApplyChangesToCRDTDoc(crdtDoc, changes) {
    const document2 = crdtDoc.getMap(CRDT_RECORD_MAP_KEY);
    Object.entries(changes).forEach(([key, value]) => {
      if (document2.get(key) !== value) {
        document2.set(key, value);
      }
    });
  }
  ...
  var serialisableBlocksCache = /* @__PURE__ */ new WeakMap();
  ...
  async function loadPostTypeEntities() {
    ...
    return Object.entries(postTypes ?? {}).map(([name, postType]) => {
      ...
      const syncedProperties = /* @__PURE__ */ new Set(["blocks"]);
      return {
        kind: "postType",
        ...
        syncConfig: {
          ...
          applyChangesToCRDTDoc: (crdtDoc, changes) => {
            const document2 = crdtDoc.getMap("document");
            Object.entries(changes).forEach(([key, value]) => {
