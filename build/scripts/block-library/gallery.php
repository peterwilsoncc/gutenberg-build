 ...
 */
function gutenberg_block_core_gallery_data_id_backcompatibility( $parsed_block ) {
...
add_filter( 'render_block_data', 'gutenberg_block_core_gallery_data_id_backcompatibility' );
 ...
 */
function gutenberg_block_core_gallery_render_context( $context, $parsed_block ) {
	if ( 'core/gallery' === $parsed_block['blockName'] ) {
...
add_filter( 'render_block_context', 'gutenberg_block_core_gallery_render_context', 10, 2 );
 ...
 */
function gutenberg_block_core_gallery_get_column_gap_value( $gap, $fallback_gap ) {
	if ( is_array( $gap ) ) {
 ...
 */
function gutenberg_block_core_gallery_is_valid_aspect_ratio( $value ) {
 ...
 */
function gutenberg_block_core_gallery_get_responsive_aspect_ratio_style_rules( $selector, $viewport_style, $media_query ) {
	if ( ! is_array( $viewport_style ) || ! is_string( $media_query ) ) {
		return array();
	}

	$aspect_ratio = $viewport_style['aspectRatio'] ?? null;
	if ( ! gutenberg_block_core_gallery_is_valid_aspect_ratio( $aspect_ratio ) ) {
		return array();
	}
 ...
 */
function gutenberg_block_core_gallery_get_responsive_flex_style_rules( $selector, $viewport_style, $media_query ) {
	if ( ! is_array( $viewport_style ) || ! is_string( $media_query ) ) {
		return array();
	}
 ...
 */
function gutenberg_block_core_gallery_resolve_dynamic_source( $source, $block ) {
	if ( ! is_array( $source ) ) {
		return array();
	}
 ...
 */
function gutenberg_block_core_gallery_dynamic_image_link_attributes( $attachment_id, $attributes ) {
 ...
 */
function gutenberg_block_core_gallery_render_dynamic_image( $attachment_id, $attributes, $context ) {
	...
	$image_attributes = array_merge(
		...
		gutenberg_block_core_gallery_dynamic_image_link_attributes( $attachment_id, $attributes )
	);
...
function gutenberg_block_core_gallery_render( $attributes, $content, $block ) {
	...
	if ( ! empty( $attributes['dynamicContent'] ) ) {
		$attachment_ids = gutenberg_block_core_gallery_resolve_dynamic_source( $attributes['dynamicContent'], $block );
		...
		foreach ( $attachment_ids as $attachment_id ) {
			$images_markup .= gutenberg_block_core_gallery_render_dynamic_image( $attachment_id, $attributes, $image_context );
		}
	...
	foreach ( $responsive_media_queries as $breakpoint => $media_query ) {
		...
		if (
			...
			gutenberg_block_core_gallery_is_valid_aspect_ratio( $viewport_style['aspectRatio'] ?? null )
		) {
	...
	if ( $is_flex_layout || $has_viewport_aspect_ratio ) {
		...
		if ( $is_flex_layout ) {
			...
			if ( null === $global_styles ) {
				$global_styles = function_exists( 'gutenberg_get_global_styles' ) ? gutenberg_get_global_styles() : array();
			}
			...
			$block_gap  = $has_block_gap
				...
				: $global_gallery_gap;
			$gap_column = gutenberg_block_core_gallery_get_column_gap_value( $block_gap, $fallback_gap );
		...
		foreach ( $responsive_media_queries as $breakpoint => $media_query ) {
			...
			if ( $is_flex_layout ) {
				...
				if ( null !== $viewport_gap ) {
					$gallery_styles[] = array(
						'selector'     => ".wp-block-gallery.{$unique_gallery_classname}",
						'declarations' => array(
							'--wp--style--unstable-gallery-gap' => gutenberg_block_core_gallery_get_column_gap_value(
								$viewport_gap,
								$fallback_gap
							),
						),
				...
				$gallery_styles = array_merge(
					$gallery_styles,
					gutenberg_block_core_gallery_get_responsive_flex_style_rules(
			...
			$gallery_styles = array_merge(
				$gallery_styles,
				gutenberg_block_core_gallery_get_responsive_aspect_ratio_style_rules(
					".{$unique_gallery_classname}",
					$viewport_style,
					$media_query
				)
...
function gutenberg_register_block_core_gallery() {
	register_block_type_from_metadata(
		__DIR__ . '/gallery',
		array(
			'render_callback' => 'gutenberg_block_core_gallery_render',
		)
	);
}
