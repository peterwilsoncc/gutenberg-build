 ...
 */
function gutenberg_block_core_table_of_contents_add_aria_label( $attributes, $content ) {
	if ( ! $content ) {
		return $content;
	}
	...
	foreach ( $nested_headings as $node ) {
		...
		if ( ! empty( $node['children'] ) ) {
			$list .= sprintf(
				'<%1$s>%2$s</%1$s>',
				$list_tag,
				gutenberg_block_core_table_of_contents_build_list_items( $node['children'], $list_tag )
			);
		}
...
function gutenberg_block_core_table_of_contents_render( $attributes, $content ) {
	...
	if ( ! is_array( $wp_current_filter ) || ! in_array( 'the_content', $wp_current_filter, true ) ) {
		return gutenberg_block_core_table_of_contents_add_aria_label( $attributes, $content );
	}
	...
	$context  = gutenberg_block_core_table_of_contents_normalize_heading_context(
		$post->post_content,
		array(
			'only_include_current_page' => ! empty( $attributes['onlyIncludeCurrentPage'] ),
			'permalink'                 => get_permalink( $post ),
			'target_page'               => gutenberg_block_core_table_of_contents_get_current_page_number(),
		)
	);
	$headings = gutenberg_block_core_table_of_contents_get_headings_from_content( $post->post_content, $max_level, $context );
	...
	$content            = sprintf(
		...
		$wrapper_attributes,
		$list_tag,
		gutenberg_block_core_table_of_contents_build_list_items(
