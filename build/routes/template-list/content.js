...
function WithMenuSeparators({ children }) {
  return import_element7.Children.toArray(children).filter(Boolean).map((child, i2) => /* @__PURE__ */ (0, import_jsx_runtime43.jsxs)(import_element7.Fragment, { children: [
...
function useDisabledDateMatchers(isValid2, parseDateFn) {
  ...
  const disabledMatchers = (0, import_element43.useMemo)(() => {
    const matchers = [];
    if (minConstraint) {
...
function DataViews({
  ...
  const _fields = (0, import_element59.useMemo)(() => normalizeFields(fields), [fields]);
...
var NavigableRegion = (0, import_element60.forwardRef)(
...
function AuthorField({ item }) {
  const [isImageLoaded, setIsImageLoaded] = (0, import_element61.useState)(false);
...
function useTemplates(activeView = "active") {
  ...
  const activeTemplates = (0, import_element62.useMemo)(() => {
  ...
  const records = (0, import_element62.useMemo)(() => {
...
function usePostTypeArchiveMenuItems() {
  ...
  const postTypesWithArchives = (0, import_element64.useMemo)(
  ...
  const postTypeLabels = (0, import_element64.useMemo)(
  ...
  const needsUniqueIdentifier = (0, import_element64.useCallback)(
    ({ labels, slug }) => {
      const singularName = labels.singular_name.toLowerCase();
...
var usePostTypeMenuItems = (onClickMenuItem) => {
  ...
  const templateLabels = (0, import_element64.useMemo)(
    () => publicPostTypes?.reduce((accumulator, { labels }) => {
  ...
  const templatePrefixes = (0, import_element64.useMemo)(
  ...
  const postTypesMenuItems = (0, import_element64.useMemo)(
    () => menuItems.reduce(
      (accumulator, postType) => {
        const { slug } = postType;
...
var useTaxonomiesMenuItems = (onClickMenuItem) => {
  ...
  const taxonomiesMenuItems = (0, import_element64.useMemo)(
    () => menuItems.reduce(
      (accumulator, taxonomy) => {
        const { slug } = taxonomy;
...
var useEntitiesInfo = (entityName, templatePrefixes, additionalQueryParameters = EMPTY_OBJECT2) => {
  ...
  const entitiesInfo = (0, import_element64.useMemo)(() => {
    return Object.keys(templatePrefixes || {}).reduce(
      (accumulator, slug) => {
        accumulator[slug] = {
...
function useSearchSuggestions(entityForSuggestions, search) {
  ...
  const query = (0, import_element65.useMemo)(
    () => ({
      order: "asc",
      context: "view",
      search,
  ...
  const [suggestions, setSuggestions] = (0, import_element65.useState)(EMPTY_ARRAY8);
...
function SuggestionList({
  ...
  const [showSearchControl, setShowSearchControl] = (0, import_element65.useState)(false);
...
function AddCustomTemplateModalContent({
  ...
  const [showSearchEntities, setShowSearchEntities] = (0, import_element65.useState)(false);
  (0, import_element65.useEffect)(() => {
...
function NewTemplateModal({ onClose }) {
  const [modalContent, setModalContent] = (0, import_element67.useState)(modalContentMap.templatesList);
  const [entityForSuggestions, setEntityForSuggestions] = (0, import_element67.useState)();
  const [isSubmitting, setIsSubmitting] = (0, import_element67.useState)(false);
...
function NewTemplate() {
  const [showModal, setShowModal] = (0, import_element67.useState)(false);
...
var add_new_template_default = (0, import_element67.memo)(NewTemplate);
...
function TemplateListActivation() {
  ...
  const [selectedRegisteredTemplate, setSelectedRegisteredTemplate] = (0, import_element68.useState)(null);
...
function useTemplatesLegacy(activeView = "all") {
  ...
  const filteredRecords = (0, import_element69.useMemo)(() => {
    if (!records) {
      return [];
    }
    if (activeView === "all") {
...
function TemplateListLegacy() {
  ...
  const activeViewOverrides = (0, import_element70.useMemo)(
    () => getActiveViewOverridesForTabLegacy(activeView),
    [activeView]
  );
  const handleQueryParamsChange = (0, import_element70.useCallback)(
    (params) => {
      navigate({
        search: {
          ...searchParams,
  ...
  const fields = (0, import_element70.useMemo)(() => {
    const elements = [];
    for (const author in users) {
      elements.push({
        value: users[author]?.id ?? author,
  ...
  const { data: posts, paginationInfo } = (0, import_element70.useMemo)(() => {
    return filterSortAndPaginate(records, view, fields);
  }, [records, view, fields]);
  const cleanupDeletedPostIdsFromUrl = (0, import_element70.useCallback)(
    (deletedItems) => {
      const deletedIds = deletedItems.map(
        (item) => item.id.toString()
      );
  ...
  const onActionPerformed = (0, import_element70.useCallback)(
    (actionId, items) => {
      if (actionId === "move-to-trash" || actionId === "permanently-delete") {
        cleanupDeletedPostIdsFromUrl(items);
      }
  ...
  const actions = (0, import_element70.useMemo)(() => {
  ...
  const handleTabChange = (0, import_element70.useCallback)(
    (viewSlug) => {
      navigate({
        to: `/templates/list/${viewSlug}`
      });
