...
function PickerActivityItem({
  ...
  const verticalGap = (0, import_element37.useMemo)(() => {
    switch (density) {
      case "comfortable":
        return "md";
      default:
...
function ViewPickerActivity({
  ...
  const { itemListLabel, paginationInfo } = (0, import_element37.useContext)(dataviews_context_default);
  const isMultiselect = useIsMultiselectPicker(actions);
  const titleField = fields.find(
    (field) => field.id === view?.titleField
  );
...
var NavigableRegion = (0, import_element74.forwardRef)(
...
function CreateTemplatePartModalContents({
  ...
  const [title, setTitle] = (0, import_element75.useState)(defaultTitle);
  const [area, setArea] = (0, import_element75.useState)(defaultArea);
  const [isSubmitting, setIsSubmitting] = (0, import_element75.useState)(false);
...
function TemplatePartList() {
  ...
  const [showTemplatePartModal, setShowTemplatePartModal] = (0, import_element76.useState)(false);
  const defaultView = DEFAULT_VIEW;
  const activeViewOverrides = (0, import_element76.useMemo)(
  ...
  const postTypeQuery = (0, import_element76.useMemo)(() => viewToQuery(view), [view]);
  const {
    records: posts,
    totalItems,
    totalPages,
